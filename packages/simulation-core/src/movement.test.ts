...
function speedOf(input: InputState): number {
  const delta = computeMovementDelta(input, DT_SECONDS);
...
describe("computeMovementDelta", () => {
  it("produces no movement when no direction is held", () => {
    const delta = computeMovementDelta({ moveX: 0, moveY: 0 }, DT_SECONDS);
    expect(delta).toEqual({ x: 0, y: 0 });
  });

  it("moves at exactly PLAYER_SPEED on a single axis", () => {
    expect(speedOf({ moveX: 1, moveY: 0 })).toBeCloseTo(PLAYER_SPEED, 6);
    expect(speedOf({ moveX: -1, moveY: 0 })).toBeCloseTo(PLAYER_SPEED, 6);
    expect(speedOf({ moveX: 0, moveY: 1 })).toBeCloseTo(PLAYER_SPEED, 6);
    expect(speedOf({ moveX: 0, moveY: -1 })).toBeCloseTo(PLAYER_SPEED, 6);
  });
  ...
  it("does not exceed PLAYER_SPEED when moving diagonally", () => {
    ...
    expect(speedOf({ moveX: 1, moveY: 1 })).toBeCloseTo(PLAYER_SPEED, 6);
    expect(speedOf({ moveX: -1, moveY: 1 })).toBeCloseTo(PLAYER_SPEED, 6);
    expect(speedOf({ moveX: 1, moveY: -1 })).toBeCloseTo(PLAYER_SPEED, 6);
    expect(speedOf({ moveX: -1, moveY: -1 })).toBeCloseTo(PLAYER_SPEED, 6);
  });
  ...
  it("scales linearly with the step duration", () => {
    const delta = computeMovementDelta({ moveX: 1, moveY: 0 }, DT_SECONDS * 2);
